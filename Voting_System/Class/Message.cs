...
namespace Voting_System.Class
{
    class Message
    {
        ...
        public void watchers(string sender, string message)
        {
            Database.DBWatchers.DBWatchers watcher = Database.DBWatchers.DBWatchers.GetData(sender);

            if (watcher != null)
            {
                watchersVotePass(message, watcher.Precinct_no, watcher.Municipality);
            }
        }

        public void watchersVotePass(string message, string precinct, string municipality)
        {
            ...
            if(words[0] == "1")
            {
                ...
                {
                    Database.DBPrecinctTabulation.DBPrecinctCongressman data = Database.DBPrecinctTabulation.DBPrecinctCongressman.GetDataByPrecinct(precinct,municipality,x);

                    Database.DBPrecinctTabulation.DBPrecinctCongressman var = new Database.DBPrecinctTabulation.DBPrecinctCongressman();

                    if (data != null)
                    {
                        var.Update(municipality, precinct, (Convert.ToInt32(words[x]) == 0) ? data.Votes : Convert.ToInt32(words[x]), x);
                    }
                    else
                    {
                        var.Add(municipality, precinct, Convert.ToInt32(words[x]), x);
                    }
                }
            }
            ...
            if (words[0] == "2")
            {
                for (int x = 1; x < arr.Length; x++)
                {
                    Database.DBPrecinctTabulation.DBPrecinctMayor data = Database.DBPrecinctTabulation.DBPrecinctMayor.GetDataByPrecinct(precinct, municipality, x);

                    Database.DBPrecinctTabulation.DBPrecinctMayor var = new Database.DBPrecinctTabulation.DBPrecinctMayor();
            ...
            if (words[0] == "3")
            {
                for (int x = 1; x < arr.Length; x++)
                {
                    Database.DBPrecinctTabulation.DBPrecinctViceMayor data = Database.DBPrecinctTabulation.DBPrecinctViceMayor.GetDataByPrecinct(precinct, municipality, x);

                    Database.DBPrecinctTabulation.DBPrecinctViceMayor var = new Database.DBPrecinctTabulation.DBPrecinctViceMayor();
            ...
            if (words[0] == "4")
            {
                for (int x = 1; x < arr.Length; x++)
                {
                    Database.DBPrecinctTabulation.DBPrecinctCouncilors data = Database.DBPrecinctTabulation.DBPrecinctCouncilors.GetDataByPrecinct(precinct, municipality, x);

                    Database.DBPrecinctTabulation.DBPrecinctCouncilors var = new Database.DBPrecinctTabulation.DBPrecinctCouncilors();
        ...
        //public int[] parsedArray(string[] array)
        //{
        //     Array arr = array;
        ...
        //    {
        //        source[x] = Convert.ToInt32(array[x+1]);
        //    }

        //    return source;
        //}

        //public void VoteParsed(string mess)
        //{
        //    try
        //    {
        //        int x = -1;

        //        string[] words = mess.Split(" ".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
        //        Array arr = words;
        ...
        //        if (arr.Length == 15)
        //        {
        //            //foreach (string parse in words)
        //            //{
        //            //    source[x++] = Convert.ToInt32(parse);
        //            //}

        //            for (x = 0; x <= 14; x++)
        //            {
        //                source[x] = Convert.ToInt32(words[x]);
        //            }

        //            for (x = 1; x <= 15 ; x++)
        //            {
        //                Database.DBTabulation.DBTabulationSummary rec = Database.DBTabulation.DBTabulationSummary.SingleDataTabulation(x);

        //                Database.DBTabulation.DBTabulationSummary up = new Database.DBTabulation.DBTabulationSummary();
        //                up.Update(x, (source[x - 1] == 0) ? rec.Votes : source[x - 1]);
        //            }
        //        }
